...
def get_board_properties(filename):
    ...
    # Device A
    deviceA1 = pcb.FindModuleByReference("J1")
    deviceA2 = pcb.FindModuleByReference("J2")
    ...
    for netclass in netclasses:
        ...
        for n in netnames:
            length = 0
            tracks_filtered = [t for t in tracks_netclass if (t.GetNet().GetNetname() == n ) and (t.IsTrack() == True)]

            pad_a = FindPadByNetName(deviceA1, n)
            if pad_a == None:
            	pad_a = FindPadByNetName(deviceA2, n)
            ...
            if n == "/DQ2":
            	print("handling /DQ2")
            	if pad_a == None:
            		print("pad_a not found")
            	if pad_b == None:
            		print("pad_b not found")

            t = None
            #start = findFirst(tracks_filtered, pad_a.GetPosition())
            #start = None
...
if __name__ == "__main__":
    ...
    while True:
        ...
        while mtime == lastmtime and not first:
            ...
            time.sleep(0.5)
            #print(f'last {lastmtime} new {mtime}')
